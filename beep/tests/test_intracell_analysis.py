...
class IntracellAnalysisTest(unittest.TestCase):
    def setUp(self):
        ...
        run = os.path.join(TEST_FILE_DIR, 'PreDiag_000220_00005E_structure_omit.json')
    ...
    def test_intracell_halfcell_matching_v2_mock(self):
        ...
        (PE_pristine_matched,
         NE_pristine_matched,
         df_real_interped,
         emulated_full_cell_interped) = ia.halfcell_initial_matching_v2(test_opt,
        ...
        self.assertAlmostEqual(PE_pristine_matched['SOC_aligned'].min(), -93.08943089430896, 5)
        self.assertAlmostEqual(PE_pristine_matched['SOC_aligned'].max(), 110.16260162601628, 5)
        self.assertAlmostEqual(PE_pristine_matched['Voltage_aligned'].min(), 2.865916, 5)
        self.assertAlmostEqual(PE_pristine_matched['Voltage_aligned'].max(), 4.299219386998656, 5)
    ...
    def test_intracell_get_dq_dv_mock(self):
        ...
        test_opt = np.array([0.999459, -4.1740795, 1.0, 0.1, 0.1])
        (PE_p_m, NE_p_m, df_real_i, em_interped) = ia.halfcell_initial_matching_v2(
            test_opt,
            real_cell_initial_charge_profile_aligned,
            ia.pe_pristine,
            ia.ne_1_pristine,
        ...
        self.assertAlmostEqual(real_cell_candidate_charge_profile_aligned["Voltage_aligned"].min(), 2.742084, 5)
        ...
        self.assertAlmostEqual(PE_out_centered["Voltage_aligned"].min(), 2.86591646, 5)
        self.assertAlmostEqual(NE_out_centered["Voltage_aligned"].min(), 0.011226739, 5)
        self.assertAlmostEqual(dVdQ_over_SOC_real["Voltage_aligned"].mean(), 3.45, 5)
        self.assertAlmostEqual(dVdQ_over_SOC_emulated["Voltage_aligned"].min(), 2.7, 5)
        self.assertAlmostEqual(dVdQ_over_SOC_emulated["Voltage_aligned"].max(), 4.2, 5)
        #
    ...
    def test_intracell(self):
        ...
        self.assertAlmostEqual(opt_result_halfcell_initial_matching.x[0], 0.999459, 5)
        self.assertAlmostEqual(opt_result_halfcell_initial_matching.x[1], -4.1740795, 6)

        # test_opt = np.array([0.999459, -4.1740795, 1.0, 0.1, 0.1])
        (PE_pristine_matched,
         NE_pristine_matched,
         df_real_interped,
         emulated_full_cell_interped) = ia.halfcell_initial_matching_v2(opt_result_halfcell_initial_matching.x,
                                                                        real_cell_initial_charge_profile_aligned,
                                                                        ia.pe_pristine,
                                                                        ia.ne_1_pristine,
                                                                        ia.ne_2_pristine_pos,
        ...
        self.assertAlmostEqual(PE_pristine_matched['SOC_aligned'].min(), -4.675029, 5)
        self.assertAlmostEqual(PE_pristine_matched['SOC_aligned'].max(), 109.350057, 5)
        self.assertAlmostEqual(PE_pristine_matched['Voltage_aligned'].min(), 2.865916, 5)
        self.assertAlmostEqual(PE_pristine_matched['Voltage_aligned'].max(), 4.29917115, 5)
...
# # initializations before for loop
        ...
        for i, cycle_index in enumerate(eol_cycle_index_list):
            ...
            degradation_optimization_result = differential_evolution(ia._get_error_from_degradation_matching,
                                                                     degradation_bounds,
...
#
            ...
            tmp_dict = {cycle_index: [LLI, LAM_PE, LAM_NE, x_NE_2, alpha_real, alpha_emulated,
                                      PE_upper_voltage, PE_lower_voltage, PE_upper_SOC, PE_lower_SOC, PE_mass,
                                      NE_upper_voltage, NE_lower_voltage, NE_upper_SOC, NE_lower_SOC, NE_mass,
                                      Li_mass
                                      ]
            ...
            real_cell_dict_of_profiles.update({cycle_index: real_cell_candidate_charge_profile_aligned})
        ...
        self.assertAlmostEqual(degradation_df['LLI'].iloc[0], -0.027076718049897863, 5)

        # summary_df = degradation_df.merge(lifetime_metric_for_cell, left_index=True, right_on='cycle_index').set_index(
        #     'cycle_index')