...
class ProcedureTest(unittest.TestCase):
    ...
    def test_generate_maccor_waveform_file_default(self):
        ...
        with ScratchDir('.') as scratch_dir:
            ...
            df_power = convert_velocity_to_power_waveform(velocity_waveform_file, 'mph')
            df_MWF = pd.read_csv(generate_maccor_waveform_file(df_power, "test_LA4_waveform", scratch_dir),
                                 sep='\t', header=None)

            # Reference mwf file generated by the cycler for the same power waveform.
            df_MWF_ref = pd.read_csv(os.path.join(TEST_FILE_DIR, "LA4_reference_default_settings.mwf"),
                                     sep="\t", header=None)
    ...
    def test_generate_maccor_waveform_file_custom(self):
        ...
        with ScratchDir('.') as scratch_dir:
            ...
            df_MWF_ref = pd.read_csv(os.path.join(TEST_FILE_DIR, "US06_reference_custom_settings.mwf"),
                                     sep="\t", header=None)
    ...
    def test_procedure_with_waveform(self):
        maccor_waveform_file = os.path.join(TEST_FILE_DIR, "LA4_reference_default_settings.mwf")
        ...
        procedure.set('MaccorTestProcedure.ProcSteps.TestStep.8', rest_step)
        procedure.set('MaccorTestProcedure.ProcSteps.TestStep.9', end_step)
        for step in procedure['MaccorTestProcedure']['ProcSteps']['TestStep']:
            print(step['Ends'])
            if step['StepType'] in ['Charge', 'Dischrge', 'Rest']:
                step['Ends']['EndEntry'][-1]['Step'] = '010'
                step['Ends']['EndEntry'][-2]['Step'] = '010'
        procedure.insert_maccor_waveform_discharge(5, maccor_waveform_file)
        steps = [x['StepType'] for x in procedure['MaccorTestProcedure']['ProcSteps']['TestStep']]
        self.assertEqual(steps, ['Rest', 'Charge', 'Rest', 'Do 1', 'Charge',
                                 'FastWave', 'AdvCycle', 'Loop 1', 'Rest', 'End'])
        self.assertEqual(procedure['MaccorTestProcedure']['ProcSteps']['TestStep'][5]['StepType'], 'FastWave')
        with ScratchDir('.') as scratch_dir:
