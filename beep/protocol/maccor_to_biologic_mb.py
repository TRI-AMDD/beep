...
class MaccorToBiologicMb:
    ...
    """
    converts seqs generated from the maccor AST to
    the biologic protocol, assumes a LATIN-1 i.e. ISO-8859-1 encoding on DOS
    """
    ...
    def maccor_ast_to_protocol_file(self, maccor_ast, filename, col_width = 20):
    ...
    def _create_loop_seq(self, num_loops, seq_num, seq_num_to_loop_to):
    ...
    def _create_advance_cyle_seqs(self, seq_num):
        ...
        if seq_num == 0:
            ...
            raise Exception("Biologic does not support advancing cycle number at step 1/seq 0")
    ...
    def _convert_time(self, time_str):
        ...
        if mins == 0.0 and secs == 0.0:
            return "{}".format(int(hours)), "hr"

        if secs == 0.0:
            total_time_min = int(hours*60 + mins)
            return "{}".format(total_time_min), "mn"

        if secs % 1.0 == 0.0:
            total_time_sec = int(hours*60*60 + mins*60 + secs)
            return "{}".format(total_time_sec), "s"
        ...
        if total_time_ms < biologic_max_time_num:
            return "{}".format(total_time_ms), "ms"
        else:
    ...
    def _unroll_loop(self, loop, num_loops, loop_start_seq_num, loop_post_seq_num):
        ...
        for i in range(1, num_loops + 1):
            for seq in loop:
                ...
                num_lims = int(seq["lim_nb"])
                for j in range(0, num_lims):
                    goto_field = "lim{}_seq".format(j + 1)
                    goto_num = int(seq[goto_field])
                    # make gotos internal to the loop work during unrolled portions
                    if loop_start_seq_num < goto_num and goto_num < loop_post_seq_num:
                        unrolled_seq[goto_field] = goto_num + (len(loop) * i)
    ...
    def _proc_step_to_seq(self, test_step, step_num, seq_from_step_num, goto_lower_bound, end_step_num):
        ...
        if num_lims > 3:
            raise Exception(
                ...
                " remove_end_entries_with_target_gotos method").format(step_num)
            )
        ...
        if num_reports > 3:
            raise Exception(
                ...
                " or pre-process the loaded maccor_ast").format(step_num)
            )
    ...
    def _create_biologic_seqs_from_maccor_ast(self, maccor_ast):
        ...
        if steps is None:
            print("Failure: could not find path: \"MaccorTestProcedure.ProcSteps.TestStep\" to steps")
            return
        ...
        for idx, step in enumerate(steps[0:-1]):
            ...
            if step_type[0:2] == "Do":
                ...
                if loop_will_unoll:
                    unrolled_loop = self._unroll_loop(loop, num_loops, loop_start_seq_num, seq_num)
                    assert len(unrolled_loop) == (len(loop) * (num_loops + 1))
                    print("loop ending at step {} unrolled to {} seqs".format(step_num, len(unrolled_loop)))
                    loop = unrolled_loop
                else:
                    loop_seq = self._create_loop_seq(num_loops, seq_num, loop_start_seq_num)
                ...
                if not adv_cycle_ignore_status_by_idx[idx]["ignore"]:
                    ...
                    blank_loop_seq, adv_cycle_loop_seq = self._create_advance_cyle_seqs(seq_num)
