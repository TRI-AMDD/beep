...
from pydash import clone_deep, get, set_
...
class MaccorToBiologicMb:
    ...
    def _convert_amps(self, val_str):
        ...
        if num < 1e-9 or decimal_sig_figs > 12:
            return f"{num * 1e12:.3f}", "pA"
        if num < 1e-6 or decimal_sig_figs > 9:
            return f"{num * 1e9:.3f}", "nA"
        elif num < 1e-3 or decimal_sig_figs > 6:
            return f"{num * 1e6:.3f}", "\N{Micro Sign}A"
        elif num < 1 or decimal_sig_figs > 3:
            return f"{num * 1e3:.3f}", "mA"
        else:
            return f"{num:.3f}", "A"
    ...
    def _convert_watts(self, val_str):
        ...
        if num < 1e-3 or decimal_sig_figs > 6:
            return f"{num * 1e6:.3f}", "\N{Micro Sign}W"
        elif num < 1 or decimal_sig_figs > 3:
            return f"{num * 1e3:.3f}", "mW"
        else:
            return f"{num:.3f}", "W"
    ...
    def _convert_ohms(self, val_str):
        ...
        if num < 1e-3 or decimal_sig_figs > 6:
            return f"{num * 1e6:.3f}", "\N{Micro Sign}Ohms"
        elif num < 1 or decimal_sig_figs > 3:
            return f"{num * 1e3:.3f}", "mOhms"
        elif num < 1e3 or decimal_sig_figs > 0:
            return f"{num:.3f}", "Ohms"
        elif num < 1e6 or decimal_sig_figs > -3:
            return f"{num * 1e-3:.3f}", "kOhms"
        else:
            return f"{num * 1e-6:.3f}", "MOhms"
    ...
    def _convert_time(self, time_str):
        ...
        if mins == 0.0 and secs == 0.0:
            return f"{int(hours):.3f}", "h"

        if secs == 0.0:
            total_time_min = int(hours * 60 + mins)
            return f"{total_time_min:.3f}", "mn"

        if secs % 1.0 == 0.0:
            total_time_sec = int(hours * 60 * 60 + mins * 60 + secs)
            return f"{total_time_sec:.3f}", "s"
        ...
        for i, step_part in enumerate(step_parts):
            ...
            if num_end_entries > 3:
                raise Exception(
                        ...
                        f"Step {step_num} has more than 3 EndEntries, the max allowed"
            ...
            for idx, end_entry in enumerate(end_entries_list):
                ...
                new_seq[f"lim{lim_num}_seq"] = goto_seq

                if goto_step_num != step_num + 1:
                    new_seq[f"lim{lim_num}_action"] = "Goto sequence"
                ...
                if end_type == "StepTime":
                    ...
                    new_seq[f"lim{lim_num}_type"] = "Time"
                    new_seq[f"lim{lim_num}_value_unit"] = lim_value_unit
                    new_seq[f"lim{lim_num}_value"] = lim_value
                    ...
                    new_seq[f"lim{lim_num}_comp"] = operator_map[end_oper]
                    new_seq[f"lim{lim_num}_type"] = "Ecell"
                    new_seq[f"lim{lim_num}_value"] = lim_value
                    new_seq[f"lim{lim_num}_value_unit"] = lim_value_unit
            ...
            if num_reports > 3:
                raise Exception(
                        ...
                        f"Step {step_num} has more than 3 ReportyEntries, the max allowed"
            ...
            for idx, report in enumerate(report_entries_list):
                ...
                if report_type == "StepTime":
                    ...
                    new_seq[f"rec{rec_num}_type"] = "Time"
                    new_seq[f"rec{rec_num}_value"] = rec_value
                    new_seq[f"rec{rec_num}_value_unit"] = rec_value_unit
                elif report_type == "Voltage":
                    rec_value, rec_value_unit = self._convert_volts(report_value)

                    new_seq[f"rec{rec_num}_type"] = "Ecell"
                    new_seq[f"rec{rec_num}_value"] = rec_value
                    new_seq[f"rec{rec_num}_value_unit"] = rec_value_unit
                elif report_type == "Current":
                    rec_value, rec_value_unit = self._convert_amps(report_value)

                    new_seq[f"rec{rec_num}_type"] = "I"
                    new_seq[f"rec{rec_num}_value"] = rec_value
                    new_seq[f"rec{rec_num}_value_unit"] = rec_value_unit
                else:
    ...
    def _seqs_to_str(self, seqs, col_width=20):
        ...
        for key in OrderedDict.keys(self._blank_seq):
            if len(key) > col_width:
                raise Exception(f"seq key {key} has length greater than col width {col_width}")

            field_row = key.ljust(col_width, " ")
            for seq_num, seq in enumerate(seqs):
                if key not in seq:
                    raise Exception(f"Could not find field {key} in seq {seq_num}")

                if len(str(seq[key])) > col_width:
                    raise Exception(f"{seq[key]} in seq {seq_num} is greater than column width")
    ...
    def _technique_to_str(self, technique_num, seqs, tech_does_loop=False, num_loops=0, col_width=20):
        ...
        if tech_does_loop:
            tech_header = f"\r\nTechnique : {technique_num + 1}\r\n"
            tech_type = "Loop\r\n"
            loop_to = "goto Ne".ljust(col_width, " ") + f"{technique_num}".ljust(col_width) + "\r\n"
            num_loops = "nt times".ljust(col_width, " ") + f"{num_loops}".ljust(col_width, " ") + "\r\n"
    ...
    def _diagnostic_file_str_from_technique_partitions_post_conversion(self, technique_partitions_post_conversion):
        ...
        safety_min_v = f"{self.min_voltage_v:.2f}" + " V"
        safety_max_v = f"{self.max_voltage_v:.2f}" + " V"
    ...
    def _apply_max_current_to_seq(self, technique_num, seq, i):
        ...
        mapped_seq["I Range Max"] = f"{self.max_current_a:.3f}"
        return mapped_seq
    ...
    def _apply_min_current_to_seq(self, technique_num, seq, i):
        ...
        mapped_seq = copy.deepcopy(seq)
        mapped_seq["I Range Min"] = f"{self.min_current_a:.3f}"
        return mapped_seq
    ...
    def _apply_max_voltage_to_seq(self, technique_num, seq, i):
        ...
        mapped_seq = copy.deepcopy(seq)
        mapped_seq["E Range Max (V)"] = f"{self.max_voltage_v:.3f}"
        return mapped_seq
    ...
    def _apply_min_voltage_to_seq(self, technique_num, seq, i):
        ...
        mapped_seq = copy.deepcopy(seq)
        mapped_seq["E Range Min (V)"] = f"{self.min_voltage_v:.3f}"
        return mapped_seq
    ...
    def _split_step(self, step, step_num):
        ...
        if limits is None:
            ...
            if not isinstance(lim, str):
                raise Exception(
                    f"Only supported Limit for StepMode {step_mode} is a single {acceptable_limit} Limit, check step {step_num}"
                )
            ...
            if step_type == "Charge":
                ...
                raise Exception(f"Unsupported StepMode {step_type} at step num {step_num}")
            ...
            raise Exception(f"Step {step_num} has Limit for step of that is not Voltage or Current")
        ...
        for i, (step, parts) in enumerate(steps_and_parts):
            ...
            if step_type in loop_open_types:
                ...
                if prev_loop_open_step_num == -1:
                    raise Exception(f"Unclosed loop at {step_num}")
            ...
            for i, step in enumerate(tp.steps):
                ...
                if step_type == "AdvCycle":
                    ...
                    if prev_loop_open != -1:
                        ...
                        raise Exception(
                            f"Loop at step {step_num} interleaved with loop at step {prev_loop_open_step_num}"
                        )
                    prev_loop_open = i
                elif step_type == allowed_loop_close:
                    if prev_loop_open == -1:
                        raise Exception(f"Unclosed loop at step {step_num}")
                    ...
                    if adv_cycle_count > 0:
                        ...
                        for j in range(prev_loop_open + 1, i + 1):
                            ...
                            if looped_step_num == step_num:
                                raise Exception(f"Loop end step {step_num} contains no measurement steps")
    ...
    def convert(self, maccor_fp, out_dir, out_filename):
        ...
        for tp, car in zip(technique_partitions_post_conversion, cycle_advancement_rules):
            advancement_rules_filename = (
                out_filename
                + f"technique_{tp.technique_num}_cycle_advancement_rules.json"
            )
            ...
            with open(advancement_rules_fp, "wb") as f:
                ...
                print(f"writing {advancement_rules_fp}")
...
def add_cycle_nums_to_csvs(
    ...
    for csv_fp, serialized_transtion_fp, csv_out_fp in technique_conversion_filepaths:
        with open(serialized_transtion_fp) as f:
            data = f.read()
