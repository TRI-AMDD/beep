...
MACCOR_TEMPLATE = loadfn(
    os.path.join(PROCEDURE_TEMPLATE_DIR, "maccor_schemas.yaml")
)
...
class BiologicMbToMaccorProcedure:
    ...
    @classmethod
    def biologic_mb_text_to_maccor_xml(cls, mps_text, maccor_header=_default_maccor_header):
        schedule_dict = Settings.mps_text_to_schedule_dict(mps_text)

        seqs = cls._get_seqs_filter_empty_at_end(schedule_dict)
    ...
    @classmethod
    def _get_seqs_filter_empty_at_end(cls, shcedule_dict):
        ...
        # parser may create empty seq
        # don't filter in case there's an empty seq in the middle
        # that would break control flow later
    ...
    def _create_step(
        ...
        step["Reports"] = (
            step["Reports"] if len(reports) == 0 else OrderedDict({"ReportEntry": reports})
        )
