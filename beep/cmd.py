...
@click.option(
    ...
    type=click.FLOAT,
    help="Threshold to drop a feature from the training dataset, in "
         "fraction of samples from 0-1. 0 = any feature having any "
         "nan is dropped, 1 = no features are dropped."
)
@click.option(
    "--train-sample-nan-thresh",
    type=click.FLOAT,
    help="Threshold to drop a sample from the training data, in "
         "fraction of features from 0-1. 0 = any sample having any"
         "nan feature is dropped, 1 = no samples are dropped."
)
@click.option(
    "--predict-sample-nan-thresh",
    type=click.FLOAT,
    help="Threshold to drop a sample from any prediction set, including "
         "those used internally (i.e., for validation, with"
)
@click.option(
...
@click.option(
    "--impute-strategy",
    type=click.STRING,
    help="Type of imputation to use, 'median' or 'mean'."
)
@click.option(
    "--kfold",
    type=click.INT,
...
@click.option(
    ...
    help="Shorten feature names to only include the featurizer name and "
         ...
         "have names including their (long) parameter hashes."
)
@click.pass_context
def train(
        ctx,
