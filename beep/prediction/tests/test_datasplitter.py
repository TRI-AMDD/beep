...
class TestBEEPDataSplitter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.features = ["::".join(c.split("::")[:2]) for c in self.sample_feature_matrix.matrix.columns if not any([feat in c for feat in ["DiagnosticProperties", "ChargingProtocol", "ExclusionCriteria"]])]
    ...
    def test_datasplitter(self):
        ...
        self.assertAlmostEqual(datasplitter.datasets[0].train_X["abs_charging_capacity::DiagnosticSummaryStats"].iloc[0], 1.2092409629538559, places=6)
    ...
    def test_split_on_charging_protocol(self):
        ...
        self.assertAlmostEqual(datasplitter.datasets[0].train_X["abs_charging_capacity::DiagnosticSummaryStats"].iloc[0], 0.955712306816296, places=6)
        ...
        for dataset in datasplitter.datasets:
            ...
            unique_split_column_values_train = split_column_values_train.apply(lambda x: "::".join([str(x[s]) for s in split_columns]), axis=1).unique()
            ...
            unique_split_column_values_test = split_column_values_test.apply(lambda x: "::".join([str(x[s]) for s in split_columns]), axis=1).unique()
